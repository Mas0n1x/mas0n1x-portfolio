...
const { notifyBotHub } = require('./botHub');
...
app.get('/api/admin/integrations', requireAuth, async (req, res) => {
  ...
  // Bot-Zentrale (Discord-Bot läuft ausgelagert)
  out.discord = process.env.BOT_HUB_URL
    ? { ok: true, status: 'Ausgelagert', detail: 'Läuft in der Bot-Zentrale' }
    : { ok: false, status: 'Bot-Zentrale nicht konfiguriert' };
  res.json(out);
});
...
function adminNotify(event, { subject, html, discordText }, def = {}) {
  ...
  // Alert an die Bot-Zentrale (feuert den Discord-Alert dort; no-op wenn nicht konfiguriert)
  if (notifyAllowed(event, 'discord', def.discord || false)) {
    notifyBotHub({ type: 'alert', title: subject, description: discordText || '' });
  }
}
...
app.post('/api/requests', requireCustomerAuth, (req, res) => {
  ...
  // Benachrichtigung an die Bot-Zentrale (Routing: default an), fire-and-forget
  if (notifyAllowed('request', 'discord', true)) {
    const customer = dbGet('SELECT name, company, email, phone FROM customers WHERE id = ?', [req.session.customerId]);
    notifyBotHub({
      type: 'request',
      request: { id: result.lastInsertRowid, project_type: projectType, budget, timeline, description },
      customer
    });
  }
